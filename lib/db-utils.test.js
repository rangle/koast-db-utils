...
var mockConfigMultiSchema = require('./mock-config-multi-schema');
...
/* the koast-db-utils is now in it's own npm package and can no longer reference a local copy of log module
so we need it passed in, here we mock a version of the log module */
function getDummyLogger() {
  return {
    verbose: function() {
      // could use console.log here for greater clarity when testing
    },
    warn: function() {
      // could use console.log here for greater clarity when testing
    },
...
describe('Testing connection.', function (done) {
  ...
  it('Create configured connections', function (done) {
    connectionPromise = dbUtils.createConfiguredConnections(null, null, mockConfig, getDummyLogger());
  ...
  it('Create another connection', function (done) {
    var promise = dbUtils.createSingleConnection('another',
      mockConfig.getConfig('anotherDb'),
      schemas);
...
describe('ability to specify an array of schemas', function () {
  ...
  before(function () {
    return dbUtils.createConfiguredConnections(null, null, mockConfigMultiSchema, getDummyLogger())
      .then(function (connections) {
